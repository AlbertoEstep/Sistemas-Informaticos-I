  ...
  <body>
    ...
    <?php 
        ...
        try {
          ...
          /* consulta */ 
          $anyomes = $_REQUEST['anio'].$_REQUEST['mes'];
          $umbral = $_REQUEST['minimo'];
          $consulta = "SELECT count(DISTINCT customerid) as cc FROM customers NATURAL JOIN orders WHERE totalamount > $umbral and TO_CHAR(orderdate, 'YYYYMM') = $anyomes";
          echo $consulta;
          ...
          if ($use_prepare) { 
              ...
              $stmt = $db->prepare("SELECT count(DISTINCT customerid)as cc FROM customers NATURAL JOIN orders WHERE totalamount > :minimo and TO_CHAR(orderdate, 'YYYYMM') = :anyomes");

              $stmt->bindparam(':minimo',$umbral, PDO::PARAM_INT);
              $stmt->bindparam(':anyomes',$anyomes, PDO::PARAM_STR);
          ...
          while($niter < $_REQUEST['iter']) {
            if ($use_prepare) {
              ...
              $stmt = $db->query("SELECT count(DISTINCT customerid)as cc FROM customers NATURAL JOIN orders WHERE totalamount > $umbral and TO_CHAR(orderdate, 'YYYYMM') = '$anyomes'");
              $linea = $stmt->fetch();
